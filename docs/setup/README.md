...
SoniqMCP defaults to no authentication (`auth_mode=none`). HTTP deployments can optionally enable static bearer token or OIDC authentication, including through the Docker Compose and Helm examples. The supported posture is local or trusted home-network use; if you expose the HTTP endpoint beyond a trusted home network, add boundary-layer protection in addition to or instead of built-in auth. See [authentication.md](authentication.md), [operations.md](operations.md), and [SECURITY.md](../../SECURITY.md) for details.
...
- [Authentication](authentication.md) — Configure `none`, `static`, or OIDC auth for HTTP deployments
