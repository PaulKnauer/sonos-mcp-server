"""Unit tests for favourites tool handlers."""
...
class FakeFavouritesService:
    ...
    def get_favourites(self) -> list[Favourite]:
        ...
        if self._raise_favourites_error:
            raise FavouritesError("favourites error")
    ...
    def play_favourite(self, room_name: str, uri: str, meta: str | None) -> None:
        ...
        if self._raise_favourites_error:
            raise FavouritesError("favourites error")
        self.play_calls.append((room_name, uri, meta))
...
class TestListFavourites:
    def test_tool_is_registered(self) -> None:
        ...
        assert get_tool(app, "list_favourites") is not None

    def test_not_registered_when_disabled(self) -> None:
        app = make_app(FakeFavouritesService(), tools_disabled=["list_favourites"])
        assert get_tool(app, "list_favourites") is None

    def test_is_read_only(self) -> None:
        app = make_app(FakeFavouritesService())
        tool = get_tool(app, "list_favourites")
        assert tool.annotations.readOnlyHint is True
        assert tool.annotations.destructiveHint is False

    @pytest.mark.anyio
    async def test_returns_favourites_list(self) -> None:
        app = make_app(FakeFavouritesService(favourites=[FAV]))
        result = await app.call_tool("list_favourites", {})
        data = parse(result)
        assert data["count"] == 1
        assert data["items"][0]["title"] == "Radio"
        assert data["items"][0]["uri"] == "x-sonosapi://radio"

    @pytest.mark.anyio
    async def test_returns_empty_list_when_no_favourites(self) -> None:
        app = make_app(FakeFavouritesService(favourites=[]))
        result = await app.call_tool("list_favourites", {})
        data = parse(result)
        assert data["count"] == 0
        assert data["items"] == []
...
class TestPlayFavourite:
    def test_tool_is_registered(self) -> None:
        app = make_app(FakeFavouritesService())
        assert get_tool(app, "play_favourite") is not None

    def test_not_registered_when_disabled(self) -> None:
        app = make_app(FakeFavouritesService(), tools_disabled=["play_favourite"])
        assert get_tool(app, "play_favourite") is None

    def test_is_control_tool(self) -> None:
        app = make_app(FakeFavouritesService())
        tool = get_tool(app, "play_favourite")
        assert tool.annotations.readOnlyHint is False

    @pytest.mark.anyio
    async def test_returns_ok_on_success(self) -> None:
        service = FakeFavouritesService(favourites=[FAV])
        app = make_app(service)
        result = await app.call_tool("play_favourite", {"room": "Living Room", "uri": FAV.uri})
        data = parse(result)
        assert data["status"] == "ok"
        assert data["room"] == "Living Room"
        assert data["uri"] == FAV.uri
        assert service.play_calls == [("Living Room", FAV.uri, None)]

    @pytest.mark.anyio
    async def test_returns_error_on_room_not_found(self) -> None:
        app = make_app(FakeFavouritesService(raise_room_not_found=True))
        result = await app.call_tool("play_favourite", {"room": "Unknown", "uri": FAV.uri})
        data = parse(result)
        assert "error" in data
